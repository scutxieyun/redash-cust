...
from redash.authentication import google_oauth, saml_auth, remote_user_auth,cas_auth
...
def get_login_url(external=False, next="/"):
    ...
    if settings.MULTI_ORG and current_org == None:
        ...
        login_url = url_for(endpoint, org_slug=current_org.slug, next=next, _external=external)
    else:
        login_url = url_for(endpoint, next=next, _external=external)
...
def get_logout_url(external=False, next="/"):
    ...
    logger.debug("logout to " + logout_url) 
    return logout_url
...
def setup_authentication(app):
    ...
    app.register_blueprint(cas_auth.blueprint)
