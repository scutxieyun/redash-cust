...
CAS_REDIRECT         = 5  #  request redirect to cas
...
def validate_cas_2(cas_host, service_url, ticket, opt):
    ...
    name = parse_tag(response,"cas:employeeName")
    orgCode = parse_tag(response,"cas:orgCode")
    email = parse_tag(response,"cas:email")
    if email is None and id is not None and orgCode is not None:
        email = id + '@' + orgCode
    ##id = parse_tag(response."cas:"
    ...
    #  Ticket validates
        ...
        return TICKET_OK, (id,name,orgCode,email)
...
def get_ticket_status(cas_host,service_url,protocol,opt):
    ...
    if ticket:
        if protocol==1:
            ...
            ticket_status, authInfo = validate_cas_2(cas_host, service_url, ticket, opt)
        #  Make cookie and return id
        if ticket_status==TICKET_OK:
            return TICKET_OK, authInfo
        #  Return error status
        else:
            return ticket_status, (None,None,None,None)
    else:
        logger.debug("no ticket found ")
        return TICKET_NONE, (None,None,None,None)
...
def login(cas_host, service_url, lifetime=None, secure=1, protocol=2, path="/", opt=""):
    ...
    cookie_status, authInfo = get_cookie_status()
    ...
    ticket_status, (id,name,orgCode,email) = get_ticket_status(cas_host,service_url,protocol,opt)
    ...
    #  Do redirect
    logger.debug("redirect the cas server")
    return CAS_REDIRECT,0,""
