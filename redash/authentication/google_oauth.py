...
def create_and_login_user(org, name, email):
    grp_ids = []
    if session.has_key('orgCode'):
        grps = models.Group.find_by_dep(org, session['orgCode'])
        if grps is not None:
            for grp in grps:
                grp_ids.append(grp.id)
        logger.debug("mapp to %r -> grps:%r with org:%r", session['orgCode'],grp_ids,org)
    try:
        ...
        if len(grp_ids) != 0:
            user_object.group_ids = grp_ids
            models.db.session.commit()
    except NoResultFound:
        logger.debug("Creating user object (%r)", name)
        if len(grp_ids) == 0:
            grp_ids.append(org.default_group.id)
        user_object = models.User(org=org, name=name, email=email, group_ids=grp_ids)
